...
/// inputs (process table, children map, hook index) on demand. One-shot — not for
/// per-refresh use, where the caller should reuse a long-lived `System` and build the
/// indexes once. Returns empty if tmux isn't running.
